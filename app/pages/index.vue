...
const { data: mostViewed, status: mvStatus } = await fetchMostViewed()

// Fetch latest articles for hero slider
const { data: latestResult, status: heroStatus } = await useFetch('/api/articles?limit=5')
const heroArticles = computed(() => (latestResult.value as any)?.data || [])

// Fetch latest articles per category for home page
const { data: categorySections, status: sectionsStatus } = await useFetch('/api/home-sections')

const isLoading = computed(() =>
  heroStatus.value === 'pending' || sectionsStatus.value === 'pending'
)
</script>
